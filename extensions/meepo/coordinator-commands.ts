/**
 * Coordinator slash commands and shortcuts. Wiring stays in coordinator.ts.
 */
...
import { lastFocusedActiveAgentId, setLastFocusedActiveAgentId, updateFleetUi } from "./coordinator-session.js";
...
import { resolveOpenAttentionFilters, resolveTaskFilters } from "./session-scope.js";
...
export interface CoordinatorCommandState {
	dashboardState: AgentsDashboardState;
	boardState: AgentsBoardState;
}

export function registerCoordinatorCommands(pi: ExtensionAPI, state: CoordinatorCommandState): void {
	...
	pi.registerCommand("agents", {
		description: "Open the child fleet dashboard",
		handler: async (_args, ctx) => {
			state.dashboardState = await runAgentsDashboard(pi, ctx, state.dashboardState);
			updateFleetUi(ctx);
		},
	});
	...
	pi.registerCommand("task-board", {
		...
		handler: async (_args, ctx) => {
			state.boardState = await runAgentsBoard(pi, ctx, state.boardState);
			updateFleetUi(ctx);
		},
	});
	...
	pi.registerCommand("agent-open", {
		description: "Focus a tracked child by id",
		handler: async (args, ctx) => {
			const id = args?.trim();
			if (!id) {
	...
	pi.registerCommand("agent-message", {
		description: "Send a structured message to a tracked child",
		handler: async (args, ctx) => {
	...
	pi.registerCommand("agent-stop", {
		description: "Stop a tracked child by id",
		handler: async (args, ctx) => {
			const parts = args?.trim().split(/\s+/) ?? [];
			const id = parts[0];
	...
	pi.registerCommand("agent-sync", {
		description: "Refresh child status from the global registry",
		handler: async (_args, ctx) => {
			try {
	...
	pi.registerCommand("service-start", {
		description: "Interactive service spawn wizard",
		handler: async (_args, ctx) => {
	...
	pi.registerCommand("services", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.hasUI) await ctx.ui.editor("services", text);
			else ctx.ui.notify(text, "info");
		},
	});

	pi.registerCommand("service-open", {
		description: "Focus a tracked service by id",
		handler: async (args, ctx) => {
			const id = args?.trim();
			if (!id) {
	...
	pi.registerCommand("service-capture", {
		description: "Capture recent service output",
		handler: async (args, ctx) => {
			const parts = args?.trim().split(/\s+/) ?? [];
			const id = parts[0];
			const lines = parts[1] ? Number(parts[1]) : 200;
	...
	pi.registerCommand("service-stop", {
		description: "Stop a tracked service by id",
		handler: async (args, ctx) => {
			const parts = args?.trim().split(/\s+/) ?? [];
			const id = parts[0];
			const force = parts.includes("force") || parts.includes("--force");
	...
	pi.registerCommand("service-sync", {
		description: "Refresh service status from the global registry",
		handler: async (_args, ctx) => {
			try {
	...
	pi.registerShortcut(Key.ctrlAlt("a"), {
		description: "Open tracked child dashboard",
		handler: async (ctx) => {
			state.dashboardState = await runAgentsDashboard(pi, ctx, state.dashboardState);
			updateFleetUi(ctx);
		},
	...
	pi.registerShortcut(Key.ctrlAlt("n"), {
		description: "Spawn a task-linked child",
		handler: async (ctx) => {
